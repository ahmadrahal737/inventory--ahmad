...
def register_user(email, password):
    ...
    conn.execute("""
        INSERT INTO users
        (email, password_hash, is_admin, is_approved, is_active)
        VALUES (?, ?, ?, ?, 1)
    """, (
        email,
...
def login_user(email, password):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT email, password_hash, is_admin, is_approved, is_active
        FROM users
        WHERE email=?
    """, (email,))
    row = cur.fetchone()
...
if not st.session_state.logged_in:
        ...
        if st.button("🔐 LOGIN", type="primary", use_container_width=True):
            if not email or not password:
                ...
                if result == "pending":
                    ...
                    st.session_state.is_admin = result["admin"]
                    st.rerun()
                else:
    ...
    with register_tab:
        ...
        if number_of_users() == 0:
            st.info("This will be the first account and will automatically become the Super Admin.")
        else:
            st.info("New accounts must be approved by the Super Admin.")
        ...
        if st.button("➕ CREATE ACCOUNT", type="primary", use_container_width=True):
            if not new_email or not new_password:
                ...
                try:
                    first = register_user(new_email, new_password)
                    if first:
                        st.success("Account created. You are the Super Admin.")
                    else:
...
if "edit_key_id" in st.session_state:
    ...
    if old:
        ...
        with st.form("edit_key_form"):
            key_name = st.text_input("KEY NAME", value=old_name or "")
            part_number = st.text_input("PART NUMBER", value=old_part or "")
            brand = st.text_input("BRAND", value=old_brand or "")
            car_model = st.text_input("CAR MODEL", value=old_model or "")
            year = st.text_input("YEAR", value=old_year or "")
            key_type = st.text_input("KEY TYPE", value=old_type or "")
            key_color = st.text_input("COLOR", value=old_color or "")
            quantity = st.number_input("QUANTITY", min_value=0, value=int(old_quantity))
            price = st.number_input("PRICE", min_value=0.0, value=float(old_price), step=0.5)
            low_limit = st.number_input("LOW STOCK ALERT", min_value=0, value=int(old_low or 5))
            notes = st.text_area("NOTES", value=old_notes or "")
            link = st.text_input("PRODUCT LINK", value=old_link or "")
            ...
            if save:
                ...
                conn.execute("""
                    ...
                    WHERE id=?
                """, (key_name, part_number, brand, car_model, year, key_type, key_color, quantity, price, low_limit, image_data, notes, link, edit_id))
                conn.commit()
                conn.close()
                ...
                conn.execute("""
                    INSERT INTO inventory_logs (user_email, action, key_id, quantity_change, new_quantity, details)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (st.session_state.user_email, "EDIT KEY", edit_id, 0, quantity, f"Updated {key_name}"))
                conn.commit()
                conn.close()
...
if "delete_key_id" in st.session_state:
    ...
    if yes.button("🗑️ YES, DELETE", use_container_width=True):
        ...
        conn.execute("""
            INSERT INTO inventory_logs (user_email, action, key_id, quantity_change, new_quantity, details)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (st.session_state.user_email, "DELETE KEY", delete_id, 0, 0, f"Deleted {key_name}"))
        conn.commit()
        conn.close()
...
# ============================================================
    ...
    st.info("Only the first account (Super Admin) can access this page.")
    conn = db()
    cur = conn.cursor()
    cur.execute("""
        SELECT email, is_admin, is_approved, is_active, created_at
        FROM users
    ...
    for email, admin, approved, active, created in users:
        with st.container(border=True):
            c1, c2, c3, c4 = st.columns([3, 1.2, 1.2, 1.2])

            with c1:
                st.write(f"**{email}**")
                if admin: st.caption("👑 SUPER ADMIN")
                elif not approved: st.caption("⏳ WAITING FOR APPROVAL")
                elif not active: st.caption("🟥 DISABLED")
                else: st.caption("✅ APPROVED USER")
            ...
            with c3:
                if not admin:
                    if active:
                        if st.button("DISABLE", key=f"disable_{email}", use_container_width=True):
                            conn = db()
                        ...
                        if st.button("ENABLE", key=f"enable_{email}", use_container_width=True):
                            conn = db()
            ...
            with c4:
                if not admin:
                    if st.button("DELETE", key=f"user_delete_{email}", use_container_width=True):
                        conn = db()
