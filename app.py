...
# (Single-admin edition: only you can log in, and you stay
#  logged in for a long time so you don't sign in every visit)
# ============================================================

DB_NAME = "car_keys.db"
LOW_STOCK_DEFAULT = 5

# --- First-time login (used ONLY the very first time the app
#     runs, to create your admin account). After that, change
#     your email/password from the "Account" page inside the
#     app instead of editing this file. ---
DEFAULT_ADMIN_EMAIL = "ahmad@example.com"
DEFAULT_ADMIN_PASSWORD = "changeme123"

# --- Used to sign the "stay logged in" cookie. Change this to
#     any random text of your own (letters/numbers), once, and
#     don't share it. ---
SECRET_KEY = "ahmad-car-keys-please-change-this-secret"

COOKIE_NAME = "ahmad_key_inventory_auth"
LOGIN_DAYS = 365  # how long "stay signed in" lasts
...
def init_database():
    ...
    # Create the one and only admin account the first time the app runs
...
def get_admin():
    ...
    row = cur.fetchone()
    conn.close()
    return row  # (email, password_hash)
